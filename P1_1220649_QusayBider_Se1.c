...
void Radix_Sort_Districts(struct District *Dis_head, int option) {
    ...
    int copy_max = max;
    for (int i = 0; i <=
                    max; i++) {                                                          //this loop using to passing through the string
...
void DeleteTown(struct District *Dis_head, int position, char District_name[], char Town_name[], int option) {
    ...
    if (current_Dis == NULL) {
        ...
        if (current_Dis->Towns != NULL) {
            ...
            if (town_delete->next_Town ==
                NULL) {                                                        // this condition  use to delete the district that existing at the end .
                town_delete->prev_Town->next_Town = town_delete->next_Town;
                free(town_delete->next_Town);
            }
            if (position_Town ==
                0) {                                                                   // this condition  use to delete the district that existing at the first .
...
void DeleteDistrict(struct District *Dis_head, int position, int option) {
    ...
    if (copy_position ==
        1) {                                                                    // this condition  use to delete the district that existing at the first .
...
int search_Town(struct District *Dis_head, char District_name[], char Town_scan[]) {
    ...
    if (Town_head == NULL)
        ...
        while (Town_head !=
               NULL) {                                                              // in this process we are passing of all town and check if the town
            ...
            if (strcmp(strlwr(Temp_Name), strlwr(Temp_Name_Scan)) ==
                0) {               // we are use strlwr to check the capitals word
